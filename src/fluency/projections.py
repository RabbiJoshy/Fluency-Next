"""How a release reads an assignment: which candidates were materialized, and
which of them may be published as an exact leaf.

Lives outside ``fluency.wsd`` because the release stage needs it whether or not
WSD ran. Portuguese ships decks with no WSD stage at all, yet the release
builder could not be imported unless the entire classifier package imported
cleanly. The module depends on nothing but the standard library, so the coupling
