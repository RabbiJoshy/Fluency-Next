...
The audit scope is exactly 200 surface-form cards and up to three selected
examples per card (600 maximum). A shortfall is published explicitly instead
of blocking the whole deck or borrowing an example from an older run. Release
activation is always a separate manual action.
...
4. `04_wsd_assignments`: optional external sense assignments, with exact method
   and model revisions required when present.
5. `05_example_selection`: up to three harvested examples per surface; this
   layer does not require WSD or change its choices when WSD is attached later.
